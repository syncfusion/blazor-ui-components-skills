# SfSmartRichTextEditor – Properties Reference
## Table of Contents
...
```razor
<SfSmartRichTextEditor Placeholder="Start to type…" />
```
...
```razor
<SfSmartRichTextEditor MaxLength="2000" ShowCharCount="true" />
```
...
Renders the editor content area inside a sandboxed `<iframe>`, isolating it from the host page's global CSS and scripts. Nest this component directly inside `<SfSmartRichTextEditor>`.
> ⚠️ **Do NOT use `EnableIFrame="true"` on `SfSmartRichTextEditor`** — that attribute does not exist. IFrame mode is configured exclusively through `<RichTextEditorIFrameSettings>`.
**Basic usage:**
```razor
<SfSmartRichTextEditor>
...
```razor
<SfSmartRichTextEditor Height="400px" />
<SfSmartRichTextEditor Height="60%" />
```
### `Width` — `string` — default: `"100%"`
Sets the editor width.
```razor
<SfSmartRichTextEditor Width="800px" />
```
...
```razor
<SfSmartRichTextEditor EnableHtmlSanitizer="true"
...
```razor
<SfSmartRichTextEditor AdditionalSanitizeTags="@(new[] { "script", "style" })" />
```
...
<SfSmartRichTextEditor DeniedSanitizeSelectors="@(new[] { "iframe[src]" })" />
```
...
```razor
<SfSmartRichTextEditor KeyConfigure="@shortcutKeys" />
@code {
...
<SfSmartRichTextEditor HttpClientInstance="@httpClient">
...
<SfSmartRichTextEditor SaveInterval="5000" AutoSaveOnIdle="true">
...
```razor
<SfSmartRichTextEditor UndoRedoSteps="50" />
```
...
<SfSmartRichTextEditor UndoRedoTimer="150" />
```
---
