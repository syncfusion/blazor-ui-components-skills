...
await rteObj.ExecuteCommandAsync(CommandName.InsertParagraph);
```
...
// Insert plain text
await rteObj.ExecuteCommandAsync(CommandName.InsertText, "typed text");
```
...
await rteObj.ShowDialogAsync(DialogType.InsertImage);
```
...
```csharp
await rteObj.CloseDialogAsync(DialogType.InsertLink);
```
